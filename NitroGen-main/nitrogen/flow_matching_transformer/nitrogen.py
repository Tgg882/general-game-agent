...
class NitroGen(torch.nn.Module):
    ...
    def __init__(
        ...
        if "siglip" in config.vision_encoder_name:
            # Offline-friendly: vision_encoder.* weights are provided by the ng.pt
            # checkpoint via load_state_dict, so downloading the full SigLIP weights
            # (>=3.5GB) is unnecessary. We only fetch the small config.json to
            # construct the architecture, then random weights get overwritten.
            from transformers import SiglipVisionConfig
            vis_config = SiglipVisionConfig.from_pretrained(config.vision_encoder_name)
            model = SiglipVisionModel(vis_config)
            # transformers>=5: SiglipVisionModel IS the vision tower itself
            # (embeddings/encoder/post_layernorm/head), matching ng.pt keys.
